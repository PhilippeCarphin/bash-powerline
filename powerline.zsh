...
nb_untracked_files(){
    ...
    for f in "${untracked[@]}" ; do
        case $f in
            */) ((dirs++)) || true ;; # can trigger errexit
            *) ((files++))  || true ;; # can trigger errexit
        esac
    done
...
__prompt(){
    ...
    #
    __prompt_section "$__host" "${c_host_bg}" "${c_host_fg}"
    if [[ -n ${PBS_JOBID} ]] ; then
    ...
    if __prompt_git_info && ! __git_ps1_ignore_repo ; then
        ...
        if [[ "${git_superproject}" != "" ]] ; then
            git_part="$(__git_ps1 " %s${git_extra} %%{\033[1;4m%%}SM%%{\033[21;24m%%} " || true)"
        else
            git_part="$(__git_ps1 " %s${git_extra}" || true)"
        fi
...
git_aggr_numstat(){
    ...
    while read ins del filename ; do
        ...
        (( total_del += del)) || true # can trigger errexit
        (( total_ins += ins)) || true # can trigger errexit
        (( total_files ++ )) || true # can trigger errexit
    ...
    while read ins del filename ; do
        ...
        (( stotal_del += del)) || true # can trigger errexit
        (( stotal_ins += ins)) || true # can trigger errexit
        (( stotal_files ++ )) || true # can trigger errexit
